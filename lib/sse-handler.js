...
export class SSEConnectionManager {
  ...
  broadcast(conversationId, eventType, data, eventId = null) {
    ...
    if (!clients || clients.size === 0) {
      console.log(`📭 No clients for ${conversationId}`);
      return;
    }
    ...
    for (const client of clients) {
      try {
        ...
        // Use message event for better compatibility
    ...
    console.log(`📤 Broadcasted to ${sentCount}/${clients.size} clients in ${conversationId}`);
